			...
			markersData = {
				...
				@if (count(@$hotels)>0)
					@foreach($hotels as $key => $data)
						{
							type_point: '{{ $location[array_search($data['hotel']['location'], array_column($location, 'id'))]['name'] }}',
							name: '{{ $data['hotel']['name'] }}',
							location_latitude: {{ $data['hotel']['lat'] }}, 
							location_longitude: {{ $data['hotel']['lng'] }},
							map_image_url: '{{ asset('uploads/hotel_'.$data['hotel']['image'].'.jpg') }}',
							rate: 'Good | {{$data['hotel']['calification']}}',
							name_point: '{{ $data['hotel']['name'] }}',
							get_directions_start_address: '{{$data['hotel']['address']}}',
							phone: '{{ $data['hotel']['phone'] }}',
							url_point: '#hotel_{{ $data['hotel']['id'] }}'
						},
					@endforeach
				@endif
				]
				...
				var mapOptions = {
					zoom: 7,
					center: new google.maps.LatLng(25.3549734, -81.1223713),
